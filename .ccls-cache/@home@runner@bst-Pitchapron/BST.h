...
void insertNode_R(TreeNodePtr * t,int value){
  if(*t==NULL){
    *t = (TreeNodePtr)malloc(sizeof(struct_TreeNode));
	 if(*t){
		(*t)->leftPtr=NULL;
		(*t)->rightPtr=NULL;
		(*t)->data=value;
...
void printftree( TreeNodePtr treePtr, int x )
{ 
   // if tree is not empty, then traverse
   if ( treePtr != NULL ) {        
        
      printftree( treePtr->rightPtr,x+1 ); //Recursion to the right

      for(int i=0;i<x;i++)printf("\t\t");
 
      printf("%5d\n",treePtr->data) ;  //print the value 
   
      printftree( treePtr->leftPtr,x+1 ); //Recursion to the left
   } // end if                          
} // end 